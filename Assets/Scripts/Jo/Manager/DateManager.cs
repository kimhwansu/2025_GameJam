...
public class DateManager : Singleton<DateManager>
{
    ...
    {
        Debug.Log("=== 게임 종료 - 엔딩 판정 ===");
        Debug.Log("7일차가 모두 끝났습니다.");

        // AchievementChecker를 통해 엔딩 체크 및 이미지 표시
        if (AchievementChecker.Instance != null)
        {
            AchievementChecker.Instance.CheckEnding();
        }
        else
        {
            Debug.LogError("AchievementChecker Instance를 찾을 수 없습니다!");
        }
    }
}